...
#define ASSERT_EQ(a, b, msg, ...)                                                                  \
	do {                                                                                       \
		if ((a) != (b)) {                                                                  \
			TEST_FAIL("Expected %ld, got %ld: " msg, (long)(b), (long)(a),             \
			          ##__VA_ARGS__);                                                  \
		}                                                                                  \
	} while (0)
...
#define ASSERT_NE(a, b, msg, ...)                                                                  \
	do {                                                                                       \
		if ((a) == (b)) {                                                                  \
			TEST_FAIL("Expected not equal to %ld: " msg, (long)(b), ##__VA_ARGS__);    \
		}                                                                                  \
	} while (0)
...
static inline int setup_bpf_test(struct bpf_test_ctx *ctx)
{
	...
	if (ctx->prog_fd < 0 || ctx->stats_fd < 0 || ctx->drop_stats_fd < 0 || ctx->config_fd < 0) {
 ...
 * @return 0 on success, -1 on error
 */
static inline int get_interface_stats(struct bpf_test_ctx *ctx, __u32 ifindex,
 ...
 * @return 0 on success, -1 on error
 */
static inline int get_drop_reason(struct bpf_test_ctx *ctx, enum drop_reason reason, __u64 *count)
{
	__u64 *percpu_count;
	__u32 key = reason;
	int cpu, err;
